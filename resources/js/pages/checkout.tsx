...
export default function Checkout({ items, total, configured }: { items: CartItem[]; total: number; configured: boolean }) {
    ...
    return (
        <StoreLayout title="Pago">
            ...
            <p className="mt-2 text-stone-500">Pagá con Mercado Pago: tarjeta de crédito, débito o transferencia bancaria.</p>

            <div className="mt-8 grid gap-4">
                {items.map((item) => (
                    <article key={item.id} className="rounded-2xl border border-amber-100 bg-white p-5">
